...
object MapsHelper {
    ...
    fun moveCamera(start: LatLng, end: LatLng, dynamicDistance:Boolean , mMap: GoogleMap,context: Context?) {
        var percentage = 0.4
    ...
    fun addMarker(mMap: GoogleMap, sourceLatLng: LatLng, animate: Boolean, icon: BitmapDescriptor, bearing:Boolean, destinationLatLng:LatLng) : Marker?{
        if (animate){
            mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(LatLng(sourceLatLng.latitude,
                sourceLatLng.longitude),
                mMap.cameraPosition.zoom))
        }

        return if (bearing){
            val addMarker = mMap.addMarker(MarkerOptions().position(sourceLatLng).icon(icon).flat(true).rotation(
                calculateBearing(sourceLatLng.latitude,sourceLatLng.longitude,destinationLatLng.latitude,destinationLatLng.longitude)))
            addMarker
        }else{
            val addMarker = mMap.addMarker(MarkerOptions().position(sourceLatLng).icon(icon).flat(true))
            addMarker
        }
    ...
    fun midpoint(lat1: LatLng, lat2: LatLng) = LatLng((lat1.latitude + lat2.latitude) / 2,
    ...
    fun directionFakePolygon(mMap: GoogleMap, addMarker: Boolean, animate: Boolean) {
        ...
        if (animate){
            mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(midpoint(LatLng(30.618958967512107,
                76.82569429976985),
                LatLng(30.649570126980876, 76.81223313046932)),
                zoom))
        }
    ...
    fun getZoomForMetersWide(
        ...
        googleMap.animateCamera(CameraUpdateFactory.newLatLngZoom(latLngPoint,
            java.lang.Float.valueOf(valToZoom.toString())), TIME_ANIMATION_MILIS, null)
    }
...
//        mMap.setOnPolygonClickListener(this)
    ...
    fun getPoints() {
        ...
        for (location in locations) {
            ...
            points.add(SphericalUtil.computeOffset(LatLng(location.latitude, location.longitude),
                accuracy.toDouble(),
                0.0))
            points.add(SphericalUtil.computeOffset(LatLng(location.latitude, location.longitude),
                accuracy.toDouble(),
